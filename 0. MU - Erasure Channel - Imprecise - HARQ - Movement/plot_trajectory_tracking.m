function plot_trajectory_tracking_comparison(conf)
    ...
    data_in = [t_arr, ones(n_pkts, 1), services, meas_errors];
...
function plot_single_tracking_subplot(completed_data, v, T_max, title_str)
    ...
    idx_show = 1:min(15, length(completed_data));
...
function completed = run_lcfs_p_tracking(data, T_max)
    ...
    last = N; finish_time = data(last,1) + data(last,3);
    if finish_time <= T_max
        completed = [completed; finish_time, data(last,1), data(last,2), data(last,4)];
    end
end
