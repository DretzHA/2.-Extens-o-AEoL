function [Sim_AEoL_NP, Sim_AEoL_P, Sim_Sys_NP, Sim_Sys_P, ...
    ...
    for j = 1:num_L
        ...
        for k_mc = 1:conf.N_MC
            ...
            % Canal Erasure (Bernoulli):
            % Probabilidade de sucesso do pacote inteiro: P_succ = (1 - delta)^k
            % Se delta for probabilidade de erro de SÍMBOLO.
            pkt_success_prob = (1 - conf.delta)^conf.k;
            
            % Vetor booleano: 1 = Sucesso, 0 = Erasure
            ...
            % NP Logic
            [aeol_np, sys_aeol_np, count_np, act_np, usr_cnt_np] = queue_logic_trajectory(...
            ...
            % P Logic
            [aeol_p, sys_aeol_p, count_p, act_p, usr_cnt_p] = queue_logic_trajectory(...
